...
public class ThreadMGFarmHarvest2 extends Thread{
	...
	public ThreadMGFarmHarvest2(ItemStack par1ItemStack, EntityPlayer ep, World par3World, int p_77648_4_, int p_77648_5_, int p_77648_6_, int p_77648_7_, float p_77648_8_, float p_77648_9_, float p_77648_10_)
	{
				...
				for(int cntX = (areaX - 1) / 2 * -1; (areaX - 1) / 2 >= cntX; cntX++)
				{
					for(int cntZ = 0; (areaZ - 1) * -1 <= cntZ; cntZ--)
					{
						...
						if(targetBlock instanceof IPlantable || targetBlock instanceof BlockBush)
						{
							...
							if(par3.getBlockMetadata(this.par4 + cntZ, this.par5+ cntY + fixY, this.par6 + cntX) >= maxMeta)
							{
				...
				for(int cntX = (areaX - 1) / 2 * -1; (areaX - 1) / 2 >= cntX; cntX++)
				{
					for(int cntZ = 0; (areaZ - 1) * -1 <= cntZ; cntZ--)
					{
						...
						if(targetBlock instanceof IPlantable || targetBlock instanceof BlockBush)
						{
							...
							if(par3.getBlockMetadata(this.par4 + cntX, this.par5+ cntY + fixY, this.par6 + cntZ) >= maxMeta)
							{
