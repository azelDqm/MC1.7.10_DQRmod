...
public class ThreadMGBreak1 extends Thread{
	...
	public ThreadMGBreak1(ItemStack par1ItemStack, EntityPlayer ep, World par3World, int p_77648_4_, int p_77648_5_, int p_77648_6_, int p_77648_7_, float p_77648_8_, float p_77648_9_, float p_77648_10_)
	{
	...
	public void run()
	{
		...
		int mode = ExtendedPlayerProperties2.get(this.ep).getToolBreak1mode();
		int area = ExtendedPlayerProperties2.get(this.ep).getToolBreak1Area();
		...
		Block touchBlock = par3.getBlock(par4, par5, par6);
			...
			for(int cntY =0; cntY < areaY; cntY++)
			{
				for(int cntX = (areaX - 1) / 2 * -1; (areaX - 1) / 2 >= cntX; cntX++)
				{
					for(int cntZ = 0; (areaZ - 1) * -1 <= cntZ; cntZ--)
					{
						...
						if(targetBlock != null && targetBlock != Blocks.air && touchBlock.getUnlocalizedName().equalsIgnoreCase(targetBlock.getUnlocalizedName()))
						{
