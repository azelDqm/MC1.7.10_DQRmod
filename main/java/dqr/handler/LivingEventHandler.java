...
import dqr.api.enums.EnumStatMamori;
import dqr.api.enums.EnumStatSubayasa;
import dqr.api.enums.EnumStatTikara;
	...
	*/
    ...
    public void onLivingUpdate(LivingUpdateEvent event) {
    	...
    	if(event.entityLiving instanceof EntityPlayer)
    	{
    		...
    		{
    			ep.addPotionEffect(new PotionEffect(DQPotionPlus.skillTouzokuSpeed.id, 60, 0));
    		}
    	...
    	}else if(event.entityLiving instanceof DqmPetBase)
    	{
    		...
    		if(pet.getJob() == 15)
    		{
    			pet.addPotionEffect(new PotionEffect(DQPotionPlus.skillTouzokuSpeed.id, 60, 0));
    		}
