...
public class ComponentSpawnerRoomOver extends StructureComponent {
	...
	public boolean addComponentParts(World world, Random random2, StructureBoundingBox structureboundingbox) {
		...
		if((world.isAirBlock(this.xPos, this.yPos, this.zPos) && world.isAirBlock(this.xPos + this.xWidth, this.yPos + 5, this.zPos + this.zWidth)) ||
		   (world.getBlock(this.xPos, this.yPos, this.zPos).getMaterial() == Material.water && world.getBlock(this.xPos + this.xWidth, this.yPos + 5, this.zPos + this.zWidth).getMaterial() == Material.water))
		{
			return false;
		}
		*/
...
//        {
        if(setChest == false)
        {
        	for(int cnt = 0; cnt < chestCnt; cnt++)
        	{
	        	...
	        	if(this.xWidth > 1 && this.zWidth > 1)
	        	{
		        	if(side == 0)
		        	{
		        		offX = 1;
		        	...
		        	if(world.isAirBlock(offX, offY, offZ) && DQR.func.isNoChestAround(world, offX, offY, offZ))
		        	{
