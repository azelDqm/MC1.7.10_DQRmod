...
public class ComponentSpawnerRoomNether extends StructureComponent {
	...
	public ComponentSpawnerRoomNether(int par1, Random par2Random2, int par3, int par4) {
		// 東西南北の方向をランダムに決める
	...
	public boolean addComponentParts(World world, Random random2, StructureBoundingBox structureboundingbox) {
		...
		/*
		if(world.isAirBlock(this.xPos, this.yPos, this.zPos) && world.isAirBlock(this.xPos + this.xWidth, this.yPos + 5, this.zPos + this.zWidth) ||
		  (world.getBlock(this.xPos, this.yPos, this.zPos).getMaterial() == Material.lava && world.isAirBlock(this.xPos + this.xWidth, this.yPos + 5, this.zPos + this.zWidth)))
		{
			return false;
		}
		*/
		...
		dam = 0;
		if(world.isAirBlock(this.xPos, this.yPos + 6, this.zPos) && world.getBlock(this.xPos, this.yPos + 5, this.zPos).getMaterial() == Material.lava) dam++;
		if(world.isAirBlock(this.xPos + this.xWidth, this.yPos + 6, this.zPos) && world.getBlock(this.xPos + this.xWidth, this.yPos + 5, this.zPos).getMaterial() == Material.lava) dam++;
		if(world.isAirBlock(this.xPos, this.yPos + 6, this.zPos + this.zWidth) && world.getBlock(this.xPos, this.yPos + 5, this.zPos + this.zWidth).getMaterial() == Material.lava) dam++;
		if(world.isAirBlock(this.xPos + this.xWidth, this.yPos + 6, this.zPos + this.zWidth) && world.getBlock(this.xPos + this.xWidth, this.yPos + 5, this.zPos + this.zWidth).getMaterial() == Material.lava) dam++;
		...
		for(int cntY = 1; cntY <= 4; cntY++)
		{
			for(int cntX = 0; cntX <= this.xWidth; cntX ++)
			{
				...
				int metaSet = 0;
				Block blc = Blocks.nether_brick;
				if(blockSet < 2)
				{
					blc = Blocks.nether_brick_fence;
					metaSet = 0;
				}else if(blockSet < 10)
				{
					metaSet = 0;
        ...
        {
        	tileentitymobspawner.func_145881_a().setEntityName(DQR.modID + "." + DQR.randomMob.getRandomNetherName());
        }
        else
        {
        	...
        	for(int cnt = 0; cnt < chestCnt; cnt++)
        	{
	        	...
	        	if(this.zWidth > 1 && this.zWidth > 1)
	        	{
		        	...
		        	if(world.getTileEntity(offX, offY, offZ) == null && DQR.func.isNoChestAround(world, offX, offY, offZ))
		        	{
			            ...
			            {
			            	DQR.randomItem.generateChestContentsRank3(random, tileentitychest);
			            	this.setChest = true;
			            }
		        	}
	        	}
