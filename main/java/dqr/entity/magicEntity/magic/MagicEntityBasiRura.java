 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityBasiRura extends MagicEntity implements IProjectile{
    ...
    protected PotionEffect potEff;
    protected int rate;
    protected int level;
    protected int[] location = new int[3];

    public MagicEntityBasiRura(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityBasiRura(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
        ...
        else//埋まってない時。速度の更新。
        {
            ...
            if (entityList != null && !entityList.isEmpty())
            {
            	for (int n = 0 ; n < entityList.size() ; n++)
            	{
            		...
            		if(this.level < 3 && flg)
            		{
            			if(target instanceof DqmNPCBase)
            			{
            				entityList.remove(n);
            				flg = false;
            			}
            		...
            		if(this.level < 1 && flg)
            		{
            			if(!(target instanceof DqmMobBase) && !(target instanceof DqmPetBase))
            			{
            				entityList.remove(n);
            				flg = false;
            			}
            		...
            		if(flg && target == this.shootingEntity)
            		{
            			entityList.remove(n);
         			}
            	}
            	...
            	for (MovingObjectPosition target : entityList)
            	{
            		if (target != null && target.entityHit != null)
            		{
                		...
                		if(target.entityHit instanceof EntityLivingBase)
                		{
                			...
                			if(pe != null)
                			{
                				elv2 = (EntityLivingBase)this.shootingEntity;
                			}
                			...
                			if(rand.nextInt(100) < this.rate)
                			{
                        		...
                        		if(elv2 instanceof DqmMobBase)
                        		{
                        			float fixRate = DQR.calcDamage.applyDamageResistMagic2(1.0F, elv2, new DamageSource("BasiRura"));
                                	Random rand2 = new Random();
                                	if((fixRate * 100) < rand2.nextInt(100))
                                    {
                                		flg2 = false;
                                    }
                        		}
                        		...
                        		{
	                				//実際に飛ばす処理
	                				World wld = elv2.worldObj;
	                				IChunkProvider provider = wld.getChunkProvider();

	                				//this.location[0] = 20000;
	                				//this.location[2] = 20000;

	                				/*
	                				int chunkX = this.location[0];
	                				int chunkY = this.location[1];
	                				int chunkZ = this.location[2];
									*/
	            					...
	            					*/
	            					...
	            					/*
	            					System.out.println("TEST1" + chunkX + "/" + chunkZ);

	            					Chunk chunk = wld.getChunkFromChunkCoords(chunkX, chunkZ);
	            					System.out.println("TEST2" + provider.chunkExists(chunkX, chunkZ));
	            					...
	            					*/
	            					...
	            					//System.out.println("TEST3" + provider.chunkExists(chunkX, chunkZ));
	                				/*
	                				Chunk chunk = wld.getChunkFromChunkCoords(this.location[0] >> 4, this.location[2] >> 4);
	                				...
	                				*/
	            					...
	            					//provider.loadChunk(chunkX, chunkZ);
	            					/*
	                				Ticket t = ForgeChunkManager.requestTicket(DQR.instance, this.worldObj, Type.NORMAL);


	                				if(t != null)
	                				{
	                					ForgeChunkManager.forceChunk(t, this.worldObj.getChunkFromBlockCoords(chunkX, chunkZ).getChunkCoordIntPair());
	                				}
	                				*/
	            					//elv2.worldObj.getChunkProvider().loadChunk(this.location[0] >> 4, this.location[2] >> 4);
	            					DqrChunkLoader.setBlockTicket(elv2.worldObj, this.location[0], this.location[1] - 2, this.location[1]);
	            					elv2.worldObj.getChunkProvider().loadChunk(this.location[0] >> 4, this.location[2] >> 4);
	                				elv2.setPositionAndUpdate(this.location[0], this.location[1] + 0.5D, this.location[2]);
	                				if(elv2 instanceof DqmPetBase)
	                				{
	                					DQR.petFunc.setUnloadPet((DqmPetBase)elv2);
	                				}
	                				elv2.worldObj.getChunkProvider().loadChunk(this.location[0] >> 4, this.location[2] >> 4);
	                				//elv2.moveForward = 1.5F;
	                                ...
	                                //ここでヒット時の効果音がなる
	                                this.playSound("dqr:player.rura", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
	                                elv2.playSound("dqr:player.rura", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
    ...
    public void setPotionEffect(PotionEffect par1)
    {
    	this.potEff = par1;
    }

    public void setRate(int par1)
    {
    	this.rate = par1;
    }
    ...
    public void setLocation(int[] par1)
    {
    	this.location = par1;
    }
}
