 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityMerazomaB extends MagicEntity implements IProjectile{
    ...
    public MagicEntityMerazomaB(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityMerazomaB(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            ...
            if (j == this.inTile && k == this.inData)
            {
                ...
                {
                	if(worldFlg && DQR.conf.magicEpMera == 1)
                	{
	                	int setX = this.xTile;
	                	int setY = this.yTile + 1;
	                	int setZ = this.zTile;

	                	for(int xCnt = -3; xCnt <= 3; xCnt++)
	                	{
	                		for(int yCnt = 3; yCnt >= -3; yCnt--)
	                		{
		            			for(int zCnt = -3; zCnt <= 3; zCnt++)
		            			{
		            				if(this.worldObj.isAirBlock(setX + xCnt, setY + yCnt, setZ + zCnt) &&
		            				   !this.worldObj.isAirBlock(setX + xCnt, setY + yCnt - 1, setZ + zCnt))
		            				{
		            					this.worldObj.setBlock(setX + xCnt, setY + yCnt, setZ + zCnt, Blocks.fire);
		            				}
		            			}
	                		}
	                	}
