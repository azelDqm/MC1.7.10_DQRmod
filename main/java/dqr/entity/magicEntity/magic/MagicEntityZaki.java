 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityZaki extends MagicEntity implements IProjectile{
    ...
    public MagicEntityZaki(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityZaki(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
            float adjustX, float adjustZ, float adjustY, float fixX, float fixZ)
   {
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            ...
            if (j == this.inTile && k == this.inData)
            {
                ...
                {
                	if(worldFlg && this.worldObj.isAirBlock(this.xTile, this.yTile + 1, this.zTile) && DQR.conf.magicEpMera == 1)
                	{
        ...
        else//埋まってない時。速度の更新。
        {
            ...
            List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this,
                            this.boundingBox.addCoord(this.motionX, this.motionY, this.motionZ).expand(1.0D + this.boxHit, 1.0D + this.boxHit, 1.0D + this.boxHit));
            double d0 = 0.0D;
            int l;
            float f1;
            ...
            for (l = 0; l < list.size(); ++l)
            {
                ...
                {
                    f1 = 0.3F;
                    AxisAlignedBB axisalignedbb1 = entity1.boundingBox.expand((double)f1 + this.boxHit , (double)f1 + this.boxHit, (double)f1 + this.boxHit);
                            	...
                            	if(rand.nextInt(100) < this.rate && (fixRate * 100) > rand2.nextInt(100))
	                            {
	                            	...
	                            	*/
	                            	...
	                            	if (elv.attackEntityFrom(ds, (float)i1))
	                                {
	                                    ...
	                                    //ここでヒット時の効果音がなる
	                                    this.playSound("dqr:mob.dokubari", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
    ...
    public void setBoxHit(int par1)
    {
    	this.boxHit = par1;
    }
}
