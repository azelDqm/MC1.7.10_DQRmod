 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityDoruma extends MagicEntity implements IProjectile{
    ...
    public MagicEntityDoruma(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityDoruma(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
            ...
            if (j == this.inTile && k == this.inData)
            {
                ...
                if (this.ticksInGround > limit)
                {
                	 List list2 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this,
     	            		this.boundingBox.addCoord(this.motionX, this.motionY, this.motionZ).expand((double)2, (double)2, (double)2));
     	            ...
     	            if (list2 != null && !list2.isEmpty())
     	            {
     	            	for (int n = 0 ; n < list2.size() ; n++)
     	            	{
                     		...
                     		if(target2 instanceof EntityLivingBase)
                     		{
                     			EntityPlayer epTarget = null;

                     			if(target2 instanceof EntityPlayer)
                     			{
                     				epTarget = (EntityPlayer)target2;

        	                        if (epTarget.capabilities.disableDamage || this.shootingEntity instanceof EntityPlayer &&
                     			...
                     			if(!(target2 instanceof EntityPlayer) || (target2 instanceof EntityPlayer && epTarget != null))
                     			{
	                     			...
	                     			if (ebl2.attackEntityFrom(damagesource2, (float)i1))
	 	                            {
	 	                                ...
	 	                                {
	 	                                    EntityLivingBase entitylivingbase = (EntityLivingBase)target2;


	 	                                    //マルチプレイ時に、両者がプレイヤーだった時のパケット送信処理
	 	                                    if (this.shootingEntity != null && ebl2 != this.shootingEntity &&
	 	                                    		ebl2 instanceof EntityPlayer && this.shootingEntity instanceof EntityPlayerMP)
	 	                                    {
	 	                                ...
	 	                                //ここでヒット時の効果音がなる
	 	                                this.playSound("dqr:mob.doruma", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
	          	            	...
	          	            	if (list2 != null && !list2.isEmpty())
	          	            	{
		          	            	for (int n = 0 ; n < list2.size() ; n++)
		          	            	{
		                          		...
		                          		if(target2 instanceof EntityLivingBase)
		                          		{
		                            		...
		                            		{
		                            			EntityLiving elv3 = (EntityLiving)target.entityHit;
		                            			pe = elv3.getActivePotionEffect(DQPotionPlus.buffMahokanta);
		                            		}
		                            		*/
		                            		...
		                            		if(pe != null && this.shootingEntity != null && this.shootingEntity instanceof EntityLivingBase)
		                            		{
		                            			EntityLiving elv = (EntityLiving)this.shootingEntity;
			                          			...
			                          			if (elv.attackEntityFrom(damagesource2, (float)i1))
			      	                            {
			      	                                ...
			      	                                {
			      	                                    //EntityLivingBase entitylivingbase = (EntityLivingBase)elv;
