 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityKiari extends MagicEntity implements IProjectile{
    ...
    public MagicEntityKiari(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityKiari(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
            	...
            	for (MovingObjectPosition target : entityList)
            	{
            		if (target != null && target.entityHit != null)
            		{
                		...
                		if(pe != null && this.shootingEntity != null && this.shootingEntity instanceof EntityLiving)
                		{
                            ...
                            {
                            	EntityLivingBase eb = (EntityLivingBase)elv;


                            	eb.removePotionEffect(Potion.poison.id);
                        		eb.removePotionEffect(DQPotionMinus.potionPoison.id);

                            	if(this.level > 0)
                            	{
                                	eb.removePotionEffect(Potion.weakness.id);
                                	eb.removePotionEffect(Potion.moveSlowdown.id);
                                	eb.removePotionEffect(Potion.digSlowdown.id);
                                	eb.removePotionEffect(Potion.harm.id);
                                	eb.removePotionEffect(Potion.confusion.id);
                                	eb.removePotionEffect(Potion.blindness.id);
                                	eb.removePotionEffect(Potion.hunger.id);
                                	eb.removePotionEffect(Potion.wither.id);
                            	}

                            	if(this.level > 1)
                            	{
                            		eb.removePotionEffect(DQPotionMinus.potionPoisonX.id);
                            		eb.removePotionEffect(DQPotionMinus.debuffHenatosu.id);
                            		eb.removePotionEffect(DQPotionMinus.debuffMedapani.id);
                            	}

                                //マルチプレイ時に、両者がプレイヤーだった時のパケット送信処理
                                if (this.shootingEntity != null && eb != this.shootingEntity &&
                                		eb instanceof EntityPlayer && this.shootingEntity instanceof EntityPlayerMP)
                                {
                            ...
                            //ここでヒット時の効果音がなる
                            this.playSound("dqr:player.kenja", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
