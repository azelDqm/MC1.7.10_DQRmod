 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityHyadoB extends MagicEntity implements IProjectile{
    ...
    protected int worldFlg = -1;

    public MagicEntityHyadoB(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityHyadoB(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
        ...
        else//埋まってない時。速度の更新。
        {
            ...
            if (entityList != null && !entityList.isEmpty())
            {
            	for (int n = 0 ; n < entityList.size() ; n++)
            	{
            		...
            		if (target instanceof EntityDragon || target instanceof EntityDragonPart ||
            		...
            		if (target != null && target.entityHit != null)
            		{
                        ...
                        else
                        {
                    		...
                    		if(target.entityHit instanceof EntityLiving)
                    		{
                    			...
                    			pe = elv2.getActivePotionEffect(DQPotionPlus.buffBreathReflect);
                    		}
                    		...
                    		if(pe != null && this.shootingEntity != null && this.shootingEntity instanceof EntityLivingBase)
                    		{
                            	...
                            	if (elv.attackEntityFrom(damagesource, (float)i1))
                                {
                                    ...
                                    //ここでヒット時の効果音がなる
                                    this.playSound("dqr:mob.hyado", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
        	...
        	if(worldFlg > -1 && DQR.conf.magicEpHyado == 1)
        	{

        		int myX = (int)this.xTile;
        		int myY = (int)this.yTile;
        		int myZ = (int)this.zTile;

        		for(int cntX = worldFlg * -1; cntX <= worldFlg; cntX++)
        		{
        			for(int cntZ = worldFlg * -1; cntZ <= worldFlg; cntZ++)
        			{
        				...
        				if(j == Blocks.water)
        				{
        					this.worldObj.setBlock(myX + cntX, myY, myZ + cntZ , Blocks.ice, 0, 2);
        	        		this.setDead();
        					//System.out.println("test3" + j.getMaterial());
        				}
        			}
        		}
            ...
            if (this.isInWater())
            {
            	...
            	if(worldFlg > -1 && DQR.conf.magicEpHyado == 1)
            	{
            		...
            		for(int cntX = worldFlg * -1; cntX <= worldFlg; cntX++)
            		{
            			...
            			{
            				Block j = this.worldObj.getBlock(myX + cntX, myY, myZ + cntZ);
            				//System.out.println("test4" + j.getMaterial());
            				//if(j.getMaterial() == Material.water)
            				if(j == Blocks.water)
            				{
            					this.worldObj.setBlock(myX + cntX, myY, myZ + cntZ , Blocks.ice, 0, 2);
            					//System.out.println("test5" + j.getMaterial());
                        		this.setDead();
            				}
            			}
            		}
    ...
    public void setWorldFlg(int par1)
    {
    	this.worldFlg = par1;
    }
}
