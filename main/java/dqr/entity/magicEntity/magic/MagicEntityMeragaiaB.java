 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityMeragaiaB extends MagicEntity implements IProjectile{
    ...
    public MagicEntityMeragaiaB(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityMeragaiaB(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
            ...
            if (j == this.inTile && k == this.inData)
            {
                ...
                if (this.ticksInGround > limit)
                {
                	if(worldFlg && DQR.conf.magicEpMera == 1)
                	{
	                	...
	                	for(int xCnt = -5; xCnt <= 5; xCnt++)
	                	{
	                		for(int yCnt = 5; yCnt >= -5; yCnt--)
	                		{
		            			for(int zCnt = -5; zCnt <= 5; zCnt++)
		            			{
