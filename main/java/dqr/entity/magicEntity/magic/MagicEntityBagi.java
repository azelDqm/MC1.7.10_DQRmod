 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityBagi extends MagicEntity implements IProjectile{
    ...
    public MagicEntityBagi(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
             ...
             */
            if(worldFlg && i.getMaterial() == Material.leaves && DQR.conf.magicEpBagi == 1)
            {
            	int x = (int)this.posX;
            	int y = (int)this.posY;
            	int z = (int)this.posZ;
            	...
            	{
            		for(int cntY = -1; cntY <= 1; cntY++)
            		{
            			for(int cntZ = -1; cntZ <= 1; cntZ++)
            			{
            				Block blc = this.worldObj.getBlock(x + cntX, y + cntY, z + cntZ);
            				if(blc.getMaterial() == Material.leaves)
            				{
            					this.worldObj.func_147480_a(x + cntX, y + cntY, z + cntZ, true);
            				}
            			}
            		}
            	}
                        ...
                        else
                        {
                    		...
                    		if(pe != null && this.shootingEntity != null && this.shootingEntity instanceof EntityLivingBase)
                    		{
                            	...
                            	if (elv.attackEntityFrom(damagesource.setDamageBypassesArmor(), (float)i1))
                                {
                                    ...
                                    //ここでヒット時の効果音がなる
                                    this.playSound("dqr:mob.bagi", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
