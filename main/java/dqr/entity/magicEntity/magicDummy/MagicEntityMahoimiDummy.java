package dqr.entity.magicEntity.magicDummy;
 ...
 * 発射されるエンティティのクラス。
 * */
public class MagicEntityMahoimiDummy extends MagicEntity implements IProjectile{
    ...
    //追加パラメータ
    protected int maxTicksRange = 30;
    ...
    public MagicEntityMahoimiDummy(World par1World)
    {
        super(par1World);
    ...
    public MagicEntityMahoimiDummy(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
            	...
            	for (MovingObjectPosition target : entityList)
            	{
            		if (target != null && target.entityHit != null)
            		{
                		...
                		if(pe != null && this.shootingEntity != null && this.shootingEntity instanceof EntityLivingBase)
                		{
                            ...
                            {
                            	EntityPlayer eb = (EntityPlayer)elv;

                            	/*
                            	int mp = ExtendedPlayerProperties.get(eb).getMP();
                            	int mpMax = ExtendedPlayerProperties.get(eb).getMaxMP();

                            	if(mp + this.damage > mpMax)
                            	{
                            		ExtendedPlayerProperties.get(eb).setMP(mpMax);
                            	}else
                            	{
                            		ExtendedPlayerProperties.get(eb).setMP((int)this.damage + mp);
                            	}

                            	if (this.damage > 70)
                            	{
                            		eb.addPotionEffect(new PotionEffect(DQR.potion.buffMahoizun.id, 60, 3));
                            	}
                            	*/
                                //マルチプレイ時に、両者がプレイヤーだった時のパケット送信処理
                                if (this.shootingEntity != null && elv != this.shootingEntity &&
                                		elv instanceof EntityPlayer && this.shootingEntity instanceof EntityPlayerMP)
                                {
                                    ((EntityPlayerMP)this.shootingEntity).playerNetServerHandler.sendPacket(new S2BPacketChangeGameState(6, 0.0F));
                                }
                            }else if (elv instanceof DqmMobBase)
                            {
                            	//DqmMobBase em = (DqmMobBase) elv;

                            	//em.DqmMobMP = em.DqmMobMP +  (int)this.damage;
                            }
                		}else
                		{
	                		//ダメージを与えることに成功したら以下の処理を行う
	                        ...
	                        {
	                        	EntityPlayer eb = (EntityPlayer)target.entityHit;
