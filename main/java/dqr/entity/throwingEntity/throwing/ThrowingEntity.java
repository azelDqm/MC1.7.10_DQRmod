 ...
 * 発射されるエンティティのクラス。
 * */
public class ThrowingEntity extends Entity implements IProjectile{
    ...
    public EntityPlayer shootingPlayer;

    public double[] posSet = new double[4];
    public float[] rotateSet = new float[2];

    public ThrowingEntity(World par1World)
    {
        super(par1World);
    ...
    public ThrowingEntity(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
        ...
        if (this.inGround)
        {
            ...
            if (j == this.inTile && k == this.inData)
            {
                ...
                if (this.ticksInGround > limit)
                {
                	...
                	/*
            		if(bumeran != null && reverse == 0)
            		{
            			bumeran.setDamage(this.damage);
            			bumeran.reverse = 1;
            			bumeran.setThisItem(this.thisItem);
            			bumeran.setShootingPlayer(this.shootingPlayer);
            			bumeran.setMaxTicksRange(this.maxTicksRange - this.ticksInAir + 2);
                    	if (!this.worldObj.isRemote)
                    	{
                    		//System.out.println("TESTTEST");
                    		this.worldObj.spawnEntityInWorld(bumeran);
                    	}
            		}else
            		{
                    	if (!this.worldObj.isRemote)
        ...
        else//埋まってない時。速度の更新。
        {
            ...
            if(this.maxTicksRange < this.ticksInAir)
            {
            	...
            	*/
            	...
            	//System.out.println("" + this.reverse + ":" + this.shootingPlayer.getDisplayName());
        		...
        		if(reverse == 0)
        		{
                   	ThrowingEntity bumeran = null;// = new ThrowingEntityBumeran(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);

                   	if(this.thisItem != null)
	                {
	                	if(this.thisItem.getItem() == DQWeapons.itemBumeran)
	                	{
	                		bumeran = new ThrowingEntityBumeran(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemYaibanobumeran)
	                	{
	                		bumeran = new ThrowingEntityYaibanobumeran(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemKurosubumeran)
	                	{
	                		bumeran = new ThrowingEntityKurosubumeran(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemIgurufeza)
	                	{
	                		bumeran = new ThrowingEntityIgurufeza(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemKazenobumeran)
	                	{
	                		bumeran = new ThrowingEntityKazenobumeran(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemHonoonobumeran)
	                	{
	                		bumeran = new ThrowingEntityHonoonobumeran(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemKoorinobumeran)
	                	{
	                		bumeran = new ThrowingEntityKoorinobumeran(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemKuresentoejji)
	                	{
	                		bumeran = new ThrowingEntityKuresentoejji(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemMeteoejji)
	                	{
	                		bumeran = new ThrowingEntityMeteoejji(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemHaipanova)
	                	{
	                		bumeran = new ThrowingEntityHaipanova(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}else if(this.thisItem.getItem() == DQWeapons.itemHaipanova2)
	                	{
	                		bumeran = new ThrowingEntityHaipanova2(this.worldObj, this, 1.0F, 1.0F, 0.0F, 0.0F, 0, -0.0F, 180.0F, posSet, rotateSet, this.shootingPlayer);
	                	}
	                }
                	...
                	if(bumeran != null)
                	{
	        			...
	        			bumeran.setThisItem(this.thisItem);
	        			bumeran.setShootingPlayer(this.shootingPlayer);
	        			bumeran.setMaxTicksRange(this.maxTicksRange + 2);
	                	if (!this.worldObj.isRemote)
	                	{
	                		//System.out.println("TESTTEST");
	                		this.worldObj.spawnEntityInWorld(bumeran);
            	...
            	for (int n = 0 ; n < entityList.size() ; n++)
            	{
            		...
            		if(this.shootingEntity instanceof EntityPlayer || this.shootingEntity instanceof EntityTameable)
            		{
	            		...
	            		if (target instanceof EntityPlayer)
	                    {
	                        ...
	                        else if (entityplayer == this.shootingEntity || entityplayer == this.shootingPlayer)
	                        {
	                        	...
	                        	if(reverse == 1)
	                        	{
	                            	if (!this.worldObj.isRemote)
	                            	{
		                        		if(entityplayer.inventory.addItemStackToInventory(this.thisItem))
		                        		{
			                            	this.setDead();
		                        		}else
		                        		{
    ...
    public boolean isDoSetDead(ThrowingEntity magic, Entity shooter )
    {
        List listX = magic.worldObj.getEntitiesWithinAABBExcludingEntity(
        		magic, magic.boundingBox.addCoord(magic.motionX, magic.motionY, magic.motionZ).
        ...
        if (listX != null && !listX.isEmpty())
        {
        	for (int n = 0 ; n < listX.size() ; n++)
        	{
        		...
        		if(shooter != null && shooter instanceof EntityDragon)
        		{
    ...
    public void setShootingPlayer(EntityPlayer ep)
    {
    	this.shootingPlayer = ep;
    }

    public void setThisItem(ItemStack item)
    {
    	this.thisItem = item;
    }
