...
public class DqmEntityNPCBukiya extends DqmNPCBase
{
    ...
    public void onLivingUpdate()
    {
		...
		*/
        ...
        if(this.itemRandom == -1 || !(this.isPotionActive(DQPotionEtc.potionCooltimeNPC) || this.isPotionActive(DQPotionEtc.potionEnableNPC)))
        {
        	//System.out.println("DEBUGLINE:==================================");
        	if(this.worldObj.isDaytime())
        	{
        		this.itemRandom = rand.nextInt(70);
        		this.addPotionEffect(new PotionEffect(DQPotionEtc.potionEnableNPC.id, 20 * 60 * 10, 0));
        	}else
        	{
        		this.addPotionEffect(new PotionEffect(DQPotionEtc.potionEnableNPC.id, 20 * 60 * 10, 1));
        		this.itemRandom = rand.nextInt(59);
        	}

        	//System.out.println("DEBUGLINE:" + this.itemRandom);
        }
    ...
    */
    ...
    public boolean interact(EntityPlayer ep)
    {
    	if(!ep.worldObj.isRemote)
    	{
    		...
    		//this.getp
    		if(this.isPotionActive(DQPotionEtc.potionCooltimeNPC) && flg == 0)
    		{
    			ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.1.txt",new Object[] {}));
    			ep.worldObj.playSoundAtEntity(ep, "dqr:player.pi", 1.0F, 1.0F);
    		}else
    		{
    			//if(this.isPotionActive(DQPotionEtc.potionEnableNPC))
    			//{
	    			if(flg == 0)
	    			{
	    				ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.0.txt",new Object[] {}));
	    			...
	    			}else if(flg == 1)
	    			{
    					...
    					tagItem = new ItemStack(DQR.bukiyaPrice.ItemOverWorld(this.itemRandom, this.getActivePotionEffect(DQPotionEtc.potionEnableNPC).getAmplifier()), 1, 0);
    					itemPrice = DQR.bukiyaPrice.GoldOverWorld(this.itemRandom, this.getActivePotionEffect(DQPotionEtc.potionEnableNPC).getAmplifier());

    					itemPrice = DQR.calcPlayerStatus.calcShoninGold(itemPrice, ep);

    					if(ExtendedPlayerProperties.get(ep).getBukiyaItem() != null && tagItem.getItem() != null && tagItem.getItem() == ExtendedPlayerProperties.get(ep).getBukiyaItem() && ep.isSneaking())
    					{
    						int epGold = ExtendedPlayerProperties.get(ep).getGold();
    						if(epGold >= itemPrice)
    						{
    		    				if(ep.inventory.addItemStackToInventory(tagItem))
    		    				{
    		    					int durLeast = this.getActivePotionEffect(DQPotionEtc.potionEnableNPC).getDuration();

    		    					ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.3.txt",new Object[] {}));

    		    					this.addPotionEffect(new PotionEffect(DQPotionEtc.potionCooltimeNPC.id, durLeast, 0));
    		    					this.removePotionEffect(DQPotionEtc.potionEnableNPC.id);
    		    					//this.addPotionEffect(new PotionEffect(DQPotionEtc.potionEnableNPC.id, 20 * 60 * 10, 1));
    		    					ExtendedPlayerProperties.get(ep).setGold(epGold - itemPrice);
		    						ExtendedPlayerProperties.get(ep).setNpcTalk(EnumDqmNpcTalk.BUKIYA.getId(), 0);
		    						ExtendedPlayerProperties.get(ep).setBukiyaItem(null);
    		    				}else
    		    				{
    		    					ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.7.txt",new Object[] {}));
    		    				}
    						}else
    						{
    							ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.6.txt",new Object[] {}));
    						}
    					}else
    					{
		    				ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.2.txt",new Object[] {tagItem.getDisplayName(),itemPrice }));
		    				ep.addChatMessage(new ChatComponentTranslation("msg.Bukiya.messages.3.txt",new Object[] {}));
		    				//ExtendedPlayerProperties.get(ep).setNpcTalk(3, 2);
		    				ExtendedPlayerProperties.get(ep).setBukiyaItem(tagItem.getItem());
    					}
	    			}
    			//}
    		}
    		...
    		int flg = ExtendedPlayerProperties.get(ep).getNpcTalk(0);
