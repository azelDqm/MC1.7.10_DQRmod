...
public class EntityPetAIAttackOnCollideJump extends EntityAIBase
{
    World worldObj;
    DqmPetBase attacker;
	...
	*/
    public EntityPetAIAttackOnCollideJump(DqmPetBase p_i1635_1_, Class p_i1635_2_, double p_i1635_3_, boolean p_i1635_5_)
    {
    ...
    public EntityPetAIAttackOnCollideJump(DqmPetBase p_i1636_1_, double p_i1636_2_, boolean p_i1636_4_)
    {
    ...
    public boolean continueExecuting()
    {
        ...
        if(rand.nextInt(10) == 0 && this.attackTick ==0)
        {
        	return false;
        }else
        {
    ...
    public void updateTask()
    {
        ...
        if ((this.longMemory || this.attacker.getEntitySenses().canSee(entitylivingbase)) && this.field_75445_i <= 0 && (this.field_151497_i == 0.0D && this.field_151495_j == 0.0D && this.field_151496_k == 0.0D || entitylivingbase.getDistanceSq(this.field_151497_i, this.field_151495_j, this.field_151496_k) >= 1.0D || this.attacker.getRNG().nextFloat() < 0.05F))
        {
            ...
            if (this.attacker.getArrayAISets(EnumDqmMobAI.JUMP.getId()) > 0)
            {
            	...
            	if (rand.nextInt(2) == 0)
                {
                    if (this.attacker.onGround)
                    {
                        ...
                        this.attacker.motionX = var4 / (double)var8 * 0.6D * 0.800000011920929D + this.attacker.motionX * 0.12D * this.attacker.getArrayAISets(EnumDqmMobAI.JUMP.getId());
                        this.attacker.motionZ = var6 / (double)var8 * 0.5D * 0.800000011920929D + this.attacker.motionZ * 0.12D * this.attacker.getArrayAISets(EnumDqmMobAI.JUMP.getId());
                        this.attacker.motionY = 0.11D * this.attacker.getArrayAISets(EnumDqmMobAI.JUMP.getId());
                    }
                }
            }
