...
public class DqmItemMagicBasiRuraC extends DqmItemMagicBase{
    private static final String __OBFID = "CL_00000072";
    public DqmItemMagicBasiRuraC(Item.ToolMaterial p_i45356_1_, float attackDam, int maxDamage, EnumDqmMagic par4)
    {
    	...
    	}else
    	{
	    	if(par1ItemStack.getItemDamage() == 0 || DQR.debug > 0)
	    	{
	    		if(!par2World.isRemote)
	    		{
					...
					{
						//位置指定テレポート
						teleX = DQR.conf.BasiRuraC_X[this.getEnumMagic().getType()];
						teleY = DQR.conf.BasiRuraC_Y[this.getEnumMagic().getType()];
						teleZ = DQR.conf.BasiRuraC_Z[this.getEnumMagic().getType()];
						int teleDim = DQR.conf.BasiRuraC_Dim[this.getEnumMagic().getType()];
		    	        ...
		    	        /*
		    	        teleX = (int)ExtendedPlayerProperties.get(par3EntityPlayer).getRuraX(this.getEnumMagic().getType());
		    	        teleY = (int)ExtendedPlayerProperties.get(par3EntityPlayer).getRuraY(this.getEnumMagic().getType());
		    	        teleZ = (int)ExtendedPlayerProperties.get(par3EntityPlayer).getRuraZ(this.getEnumMagic().getType());
						*/
    ...
    @Override
  	 public void addInformation(ItemStack p_77624_1_, EntityPlayer p_77624_2_, List p_77624_3_, boolean p_77624_4_) {
    	...
    	PacketHandler.INSTANCE.sendToServer(new MessageServerFunction(EnumDqmFuncPacketCode.SyncCBasirura));

    	int setDim = (int)ExtendedPlayerProperties3.get(p_77624_2_).getC_BasiRuraDim(this.getEnumMagic().getType());
        double setX = Math.floor(ExtendedPlayerProperties3.get(p_77624_2_).getC_BasiRuraX(this.getEnumMagic().getType()));
        double setY = Math.floor(ExtendedPlayerProperties3.get(p_77624_2_).getC_BasiRuraY(this.getEnumMagic().getType()));
        double setZ = Math.floor(ExtendedPlayerProperties3.get(p_77624_2_).getC_BasiRuraZ(this.getEnumMagic().getType()));
