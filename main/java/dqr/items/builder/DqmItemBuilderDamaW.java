...
import dqr.thread.ThreadBuilderDamaW;
import dqr.thread.ThreadBuilderFrame;

public class DqmItemBuilderDamaW extends DqmItemBuilderBase{

	public DqmItemBuilderDamaW(EnumDqmBuilder par1)
	{
		super(par1);
	}
    ...
    public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
    {
    	...
    	if(!par3World.isRemote)
    	{
			...
			if(DQR.conf.permBuilder3 == 2 || (DQR.conf.permBuilder3 == 1 && flg))
			{
				;
			}else
			{
    	...
    	}else
    	{
        	...
        	if(nbt == null || nbt.getInteger("buildReady") == -1)
        	{
        		if(DQR.conf.needBuilderFrame3 == 1)
        		{
        			if(!par3World.isRemote)
        			{
