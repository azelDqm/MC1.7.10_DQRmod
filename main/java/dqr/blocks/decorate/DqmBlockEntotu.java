...
public class DqmBlockEntotu extends BlockContainer
{
    public static String setmodel;
    private String model;
    private float rotationYaw;

    public DqmBlockEntotu(Material var2)
    {
        super(var2);
        //光源
    ...
    {
        this.blockIcon = var1.registerIcon("dqr:EntotuB");
    }
    //モデル
    public DqmBlockEntotu setmodel(String m)
    {
        model = m;
        return this;
    }
    ...
    public TileEntity createNewTileEntity(World var1, int par2)
    {
        if (model == "Entotu")
        {
            return new DqmTileEntityEntotu();
        }

        if (model == "EntotuG")
        {
            return new DqmTileEntityEntotuG();
        }

        if (model == "EntotuS")
        {
            return new DqmTileEntityEntotuS();
        }

        if (model == "EntotuN")
        {
            return new DqmTileEntityEntotuN();
        }

        if (model == "EntotuO")
        {
            return new DqmTileEntityEntotuO();
        }
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if (Blocks.fire.canCatchFire(par1World, par2 - 1, par3, par4, ForgeDirection.EAST))
        {
            for (l = 0; l < 2; ++l)
            {
                ...
                par1World.spawnParticle("largesmoke", (double)f, (double)f1 + 0.5F, (double)f2, 0.0D, 0.0D, 0.0D);
            }
        }
